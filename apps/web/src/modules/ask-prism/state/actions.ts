...
>(`${sliceName}/ask-question`, async (payload, { extra }) => {
	...
	try {
		...
		// Development fallback while backend endpoint is being implemented
		...
		return {
			answer: `Based on the project knowledge base, here is the answer regarding "${payload.query}":\n\nThe system enforces strict validation and access control. Passwords require a minimum length and complexity check. All knowledge node modifications are tracked with versioning, and AI proposals must be human-validated before integration.`,
			sources: [
				{
					id: "source-1",
					nodeId: "auth-rules",
